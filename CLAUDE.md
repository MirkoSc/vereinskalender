...
Läuft auf all-inkl Shared Hosting (PHP + MySQL). Dieses Dokument ist die
...
- Shared Hosting bei all-inkl: **kein SSH, kein Git, kein Composer, kein `exec()`/`shell_exec()`** auf dem Server.
...
- **Zielversion PHP 8.5** – aller Code und alle Abhängigkeiten müssen unter
  PHP 8.5 fehler- und deprecation-frei laufen (im KAS wird PHP 8.5 eingestellt).
...
```
/web/                DocumentRoot (im KAS eingestellt, wird NIE geändert)
...
- Cronjob im all-inkl KAS ruft `bin/import_ics.php` alle 10 Minuten per HTTP auf
...
- **Rechtsseiten**: Impressum und Datenschutzerklärung als
  ...
  Anonymisierung), Web-Push-Abos, Hosting bei all-inkl. Footer-Links auf
  jeder Seite.
...
- Lokale Umgebung: `docker-compose.yml` mit **PHP 8.5** + MySQL, all-inkl-nah
